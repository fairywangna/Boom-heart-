...
前面dom 都是通过js 来 操作的，  html 5  也有新的操作dom  （但是 跟CSS3一样，高版本浏览器才能执行的）

## 1. 获取元素：

a) document.getElementsByClassName ('class') 通过类名获取元素，以伪数组形式存在。

b) document.querySelector('selector') 通过CSS选择器获取元素，符合匹配条件的第1个元素。

c) document.querySelectorAll('selector') 通过CSS选择器获取元素，以伪数组形式存在。
